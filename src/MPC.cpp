  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 1; t < N; t++) {
      ...
      */
      ...
      fg[1 + x_start + t] = x1 - (x0 + v0 * mph2mps * CppAD::cos(psi0) * dt);

      fg[1 + y_start + t] = y1 - (y0 + v0 * mph2mps * CppAD::sin(psi0) * dt);     

      fg[1 + psi_start + t] = psi1 - (psi0 + v0 * mph2mps * delta0 / Lf * dt);
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[1 + cte_start + t] =
          cte1 - ((f0 - y0) + (v0 * mph2mps * CppAD::sin(epsi0) * dt));
      fg[1 + epsi_start + t] =
          epsi1 - ((psi0 - psides0) + v0 * mph2mps * delta0 / Lf * dt);
    }
  }
};
